...
const EMPTY_SPEC = { code: '', name: '', description: '', isActive: true };
const EMPTY_LEVEL = { code: '', name: '', levelOrder: 1, description: '', isActive: true };
...
export default function SpecializationFormModal({ open, mode, onClose, onSubmit, initialData = null, loading = false }) {
    ...
    if (open) {
      if (initialData) {
        setForm({
          ...
          ...(isLevelMode && { levelOrder: Number(initialData?.levelOrder ?? 1) })
        });
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(15,23,42,0.4)', backdropFilter: 'blur(3px)' }}>
      ...
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 shrink-0">
          <h2 className="text-xs font-bold text-slate-800">
            ...
            {mode === 'LEVEL_ADD' && 'Thêm cấp độ tầng mức mới'}
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3.5 text-xs">
          ...
          <div className={isLevelMode ? "grid grid-cols-3 gap-3" : "block"}>
            <div className={isLevelMode ? "col-span-2 flex flex-col gap-1" : "flex flex-col gap-1"}>
              <label className="flex items-center gap-1 text-[11px] font-semibold text-slate-600">
                <Code size={13} /> Mã định danh Code <span className="text-rose-500">*</span>
              </label>
              <input
                type="text"
            ...
            {isLevelMode && (
              <div className="flex flex-col gap-1">
                <label className="flex items-center gap-1 text-[11px] font-semibold text-slate-600">
                  <ArrowUpDown size={13} /> Thứ tự bậc
                </label>
                <input
                  type="number"
                  min={1}
          ...
          <div className="flex flex-col gap-1">
            <label className="flex items-center gap-1 text-[11px] font-semibold text-slate-600">
              <FileText size={13} /> Tên gọi hiển thị <span className="text-rose-500">*</span>
            </label>
            <input
              type="text"
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-semibold text-slate-600">Mô tả tóm tắt</label>
            <textarea
          ...
          <div className="flex items-center gap-2 pt-1 select-none">
            ...
            />
            <label htmlFor="activeCheck" className="text-[11px] font-semibold text-slate-600 cursor-pointer">
        ...
        <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-slate-100 bg-slate-50/60 shrink-0">
          <button type="button" onClick={onClose} disabled={loading} className="px-3 py-1.5 text-xs font-medium text-slate-600 rounded-lg bg-white border border-slate-200">
            Hủy
          </button>
          <button
            type="button"
            ...
            style={{ background: isLevelMode ? '#059669' : '#1b3392' }}
          >
            Lưu
          </button>
        </div>
